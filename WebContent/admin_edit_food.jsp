...
<body>
	...
	<div class="row">
		...
		<div class="col-md-8">
			...
			%>
			<form role="form" method="post" action="admin_edit_foodAction.jsp">
			<input type="hidden" value="<%=f.getId()%>" name="id">
			<div class="panel-body">
				<div class="form-group">
					<label for="foodname">菜品名称</label>
					<input type="text" class="form-control" name="fn" id="foodname" required="required" value="<%=f.getFoodName() %>" />
				</div>
				<div class="form-group">
					<label for="feature">菜品特色</label>
					<textarea class="form-control" rows="3" name="fea" id="feature" required="required"><%=f.getFeature() %></textarea>
				</div>
				<div class="form-group">
					<label for="material">主要原料</label>
					<textarea class="form-control" rows="3" name="mat" id="material" required="required"><%=f.getMaterial() %></textarea>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<input type="number" class="form-control" name="price" id="price" required="required" value="<%=f.getPrice() %>"/>
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="com" id="comment" value="<%=f.getComment() %>" required="required" />
					<p class="help-block">-1代表正常菜，0代表厨师推荐，正整数代表特价菜价格。</p>
				</div>
			</div>
			<div class="panel-footer text-center">
				<button type="submit" class="btn btn-warning">确认修改</button>&nbsp;&nbsp;
				<a role="button" class="btn btn-default" href="admin_list_food">放弃返回</a>
			</div>
			</form>
